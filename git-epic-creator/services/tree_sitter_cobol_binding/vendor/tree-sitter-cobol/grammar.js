function sepBy(pattern, separator) {
  ...
  module.exports = grammar({
    ...
    conflicts: $ => [
      // Grammar ambiguity: INVOKE <target> <number> . can be interpreted as
      // an invoke target list or as a numeric literal. We explicitly allow
      // this conflict so generation can proceed; corpus tests must guard
      // resulting parse quality.
      [$.invoke_statement, $.number],
      // Grammar ambiguity: compiler directives can begin with CBL/PROCESS and
      // then include integers (options), which overlaps with generic number
      // handling in some contexts during generation.
      [$.compiler_directive, $.number],
      // Grammar ambiguity: EXEC blocks accept integers inside their bodies,
      // which can overlap with generic number handling during generation.
      [$.execute_body, $.number],
    ],
    ...
    rules: {
      ...
      // Compiler-directing statements (IBM/MF) are not COBOL proper, but appear
      // in real enterprise repos. We parse them as top-level nodes so they do
      // not cause ERROR cascades. The consuming pipeline should still preserve
      // them as evidence and (where needed) interpret them deterministically.
      // NOTE: We intentionally do *not* try to capture the whole line as a single
      // token because whitespace is handled via externals/extras in this grammar.
      // Instead we accept: CBL/PROCESS <some tokens> ... until the next division.
      compiler_directive: $ => seq(
        choice($._CBL, $._PROCESS),
        repeat1(choice(
          $.WORD,
          $._LITERAL,
          $.integer,
      ...
      _identifier: $ => prec.right(choice(
        // IBM Enterprise COBOL special register.
        $._RETURN_CODE,
        $.qualified_word,
      ...
      // IBM Enterprise COBOL (z/OS) extension / widely used enterprise syntax:
      // Keep these rules intentionally permissive to avoid ERROR cascades.
      ...
      // IBM Enterprise COBOL frequently embeds non-COBOL languages via EXEC blocks,
      // e.g. EXEC SQL / EXEC CICS / EXEC DLI ... END-EXEC.
      // We keep this permissive to avoid ERROR cascades in real codebases.
      execute_body: $ => repeat1(choice(
        $.WORD,
        $._LITERAL,
        $.integer,
        // Common punctuation/operators seen inside EXEC blocks (SQL/CICS/IMS).
        '(', ')', ':', '=', '+', '-', '*', '/', '.', '<', '>', 
      )),
      ...
      // Give directive keywords higher lexical precedence than generic WORD so
      // they are recognized deterministically at file start.
      _CBL: $ => token(prec(10, /[cC][bB][lL]/)),
      _CH: $ => /[cC][hH]/,
      ...
      _END_EXECUTE: $ => token(prec(10, /[eE][nN][dD]-[eE][xX][eE][cC][uU][tT][eE]/)),
      ...
      _NEXT: $ => /[nN][eE][xX][tT]/,
      _EXECUTE: $ => token(prec(10, /[eE][xX][eE][cC][uU][tT][eE]/)),
      ...
      _PROCESS: $ => token(prec(10, /[pP][rR][oO][cC][eE][sS][sS]/)),
      ...
      _RETURN_CODE: $ => token(prec(10, /[rR][eE][tT][uU][rR][nN]-[cC][oO][dD][eE]/)),
