...
class DoclingProcessor:
    ...
    def _docling_artifacts_path(self) -> str | None:
        """Return artifacts path if configured and exists on disk."""
        artifacts_root = (self.settings.DOCLING_ARTIFACTS_PATH or "").strip()
        return artifacts_root if artifacts_root and os.path.isdir(artifacts_root) else None

    def _images_scale(self) -> float:
        return float(self.settings.DOCLING_IMAGES_SCALE)

    def _vlm_log_context(self) -> Dict[str, Any]:
        provider = (self.settings.DOCLING_VLM_PROVIDER or "").lower()
        is_azure = provider == "azure_openai"
        return {
            "vlm_mode": self.settings.DOCLING_VLM_MODE,
            "vlm_provider": self.settings.DOCLING_VLM_PROVIDER,
            "endpoint": self.settings.DOCLING_AZURE_OPENAI_ENDPOINT if is_azure else self.settings.DOCLING_VLM_ENDPOINT,
            "deployment": self.settings.DOCLING_AZURE_OPENAI_DEPLOYMENT_NAME if is_azure else None,
        }

    def _return_vlm_api_error(
        self,
        *,
        event: str,
        file_path: str,
    ...
    def _create_rapidocr_options(self) -> RapidOcrOptions:
        ...
        """
        models_path = (self.settings.RAPIDOCR_MODELS_PATH or "").strip()
        font_path = self._get_font_path()
        ocr_langs = (self.settings.DOCLING_OCR_LANGS or "").strip()
        lang_list = [lang.strip() for lang in ocr_langs.split(",") if lang.strip()]

        # If a valid models directory is configured, use explicit paths for offline operation.
        # Otherwise, let RapidOCR fall back to its own defaults (better for local dev/tests).
        det_model = os.path.join(models_path, "ch_PP-OCRv3_det_infer.onnx") if models_path else None
        rec_model = os.path.join(models_path, "ch_PP-OCRv3_rec_infer.onnx") if models_path else None
        cls_model = os.path.join(models_path, "ch_ppocr_mobile_v2.0_cls_infer.onnx") if models_path else None
        if models_path and not os.path.isdir(models_path):
            det_model = rec_model = cls_model = None
    ...
    def _create_pipeline_options_base(
        self,
        *,
        ...
        artifacts_path = self._docling_artifacts_path()
        opts = PdfPipelineOptions(artifacts_path=artifacts_path)
        opts.do_ocr = bool(self.settings.DOCLING_USE_OCR)
        opts.do_picture_description = True
        opts.picture_description_options = picture_description_options
        opts.images_scale = images_scale
        opts.generate_picture_images = True
        opts.enable_remote_services = enable_remote_services
        opts.do_picture_classification = do_picture_classification
        opts.ocr_options = self._create_rapidocr_options()
        return opts
    ...
    def _create_image_pipeline_options(
        ...
        """
        # Note: Docling currently expects PdfPipelineOptions even for image processing.
        return self._create_pipeline_options_base(
    ...
    def _create_pdf_pipeline_options(
        self,
        *,
        picture_description_options: Any,
        images_scale: float,
    ...
    def _new_picture_description_api_options(
        self,
        *,
        url: str,
    ...
    def _create_openai_compatible_chat_completions_options(
        self,
        *,
        url_base: str,
        model: str,
        ...
        if include_max_tokens:
            params["max_tokens"] = self.settings.DOCLING_VLM_MAX_TOKENS
        if include_temperature:
            return self._new_picture_description_api_options(
    ...
    def _create_azure_openai_vlm_options(self) -> PictureDescriptionApiOptions:
        ...
        headers = {"api-key": self.settings.DOCLING_AZURE_OPENAI_API_KEY}
        
        logger.info("AZURE_OPENAI_VLM_CONFIGURED",
                   endpoint=base_url,
                   deployment=deployment,
                   api_version=api_version,
                   response_format=self.settings.DOCLING_VLM_RESPONSE_FORMAT)
        return self._new_picture_description_api_options(
            url=url,
            params={
    ...
    def _create_lm_studio_vlm_options(self) -> PictureDescriptionApiOptions:
        ...
        return self._create_openai_compatible_chat_completions_options(
            url_base=self.settings.DOCLING_VLM_ENDPOINT,
            model=self.settings.DOCLING_VLM_MODEL,
    ...
    def _create_watsonx_vlm_options(self) -> PictureDescriptionApiOptions:
        ...
        return self._new_picture_description_api_options(
            ...
            params={
                ...
                "project_id": self.settings.WX_PROJECT_ID,
                "parameters": {"max_new_tokens": self.settings.DOCLING_VLM_MAX_TOKENS},
            },
            headers={"Authorization": "Bearer " + _get_iam_access_token(api_key=self.settings.WX_API_KEY)},
        )
    ...
    def extract_text_with_result(self, file_path: str) -> DocumentProcessingResult:
        ...
        try:
            ...
            # Standalone images often yield no PictureItem elements, meaning Docling's
            # picture-description enrichment never runs. If OCR is empty, we still want
            # a VLM-based description for diagrams/photos.
            if (not is_pdf) and (not extracted_text.strip()):
                try:
                    logger.warning(
                        "IMAGE_EMPTY_TEXT_DETECTED",
                        file_path=file_path,
                        file_size_bytes=file_size_bytes,
                        strategy="direct_image_description",
                        vlm_mode=self.settings.DOCLING_VLM_MODE,
                    )
                    desc = self._describe_image_direct(file_path)
                    if desc and desc.strip():
                        extracted_text = desc.strip()
                        metadata = {**metadata, **{"image_description_used": True}}
                        logger.info(
                    ...
                    logger.error(
                        ...
                        file_path=file_path,
                        error=str(desc_exc),
                        error_type=type(desc_exc).__name__,
                        vlm_mode=self.settings.DOCLING_VLM_MODE,
                        exc_info=True,
                    )
            ...
            return self._return_vlm_api_error(
                event="DOCLING_VLM_API_TIMEOUT",
                file_path=file_path,
                error_msg=error_msg,
                exc=timeout_exc,
                extra={"timeout": self.settings.DOCLING_VLM_TIMEOUT},
            )
            ...
            return self._return_vlm_api_error(
                event="DOCLING_VLM_API_HTTP_ERROR",
                file_path=file_path,
                error_msg=error_msg,
                exc=http_exc,
            ...
            return self._return_vlm_api_error(
                event="DOCLING_VLM_API_CONNECTION_ERROR",
                file_path=file_path,
                error_msg=error_msg,
                exc=conn_exc,
            )
            ...
            return self._return_vlm_api_error(
                event="DOCLING_VLM_API_REQUEST_ERROR",
                file_path=file_path,
                error_msg=error_msg,
                exc=req_exc,
            )
    ...
    def _build_converter(self) -> DocumentConverter:
        ...
        vlm_mode = (self.settings.DOCLING_VLM_MODE or "").lower()

        if vlm_mode == "local":
            enable_remote_services = False
            provider_label = "SmolVLM"
            picture_description_options = smolvlm_picture_description
            strategy = "local_pdf_with_smolvlm"
        elif vlm_mode == "remote":
            enable_remote_services = True
            provider_label = self.settings.DOCLING_VLM_PROVIDER
        ...
        pdf_opts = self._create_pdf_pipeline_options(
            picture_description_options=picture_description_options,
            images_scale=images_scale,
            enable_remote_services=enable_remote_services,
        )
    ...
    def _describe_image_direct(self, file_path: str) -> str:
        """
        Generate a VLM-based description for a standalone image.

        Docling's built-in picture-description enrichment operates on PictureItem elements.
        For standalone images with no detected PictureItem (common when OCR is empty),
        we run the configured picture description model directly on the full image.
        """
        ...
        vlm_mode = (self.settings.DOCLING_VLM_MODE or "local").lower()
        enable_remote_services = vlm_mode == "remote"

        # Prefer pre-downloaded artifacts if present, otherwise allow HF download/cached usage.
        artifacts_root = self.settings.DOCLING_ARTIFACTS_PATH
        artifacts_path = artifacts_root if artifacts_root and os.path.isdir(artifacts_root) else None

        if enable_remote_services:
            options = self._get_remote_vlm_options()
        else:
        ...
        out = next(iter(model._annotate_images([img])))  # type: ignore[attr-defined]
