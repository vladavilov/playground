...
WITH p, 
     ...
     count(CASE WHEN ch.embedding IS NOT NULL AND size(ch.embedding) = 1536 THEN 1 END) AS chunks_with_embedding

// Entities
OPTIONAL MATCH (e:__Entity__)-[:IN_PROJECT]->(p)
WITH p, total_chunks, chunks_with_embedding,
     count(e) AS total_entities,
     count(CASE WHEN e.embedding IS NOT NULL AND size(e.embedding) = 1536 THEN 1 END) AS entities_with_embedding
...
RETURN 
  ...
  count(CASE WHEN c.embedding IS NOT NULL AND size(c.embedding) = 1536 THEN 1 END) AS communities_with_embedding,
  count(c) - count(CASE WHEN c.embedding IS NOT NULL AND size(c.embedding) = 1536 THEN 1 END) AS communities_missing_embedding,
  // Overall health metrics
  CASE 
    WHEN count(c) = 0 THEN 0.0  // No communities = critical
    ELSE toFloat(count(CASE WHEN c.embedding IS NOT NULL AND size(c.embedding) = 1536 THEN 1 END)) / count(c)
