...
WITH value, 
     toUpper(coalesce(value.norm_title, value.title)) AS norm_title, 
...
WITH value, norm_title, description, e_id, e_nt, e_desc
ORDER BY e_desc.id
WITH value, norm_title, description, e_id, e_nt, head(collect(e_desc)) AS e_desc

// Select existing entity by priority or use value.id for new entity
WITH value, norm_title, description, coalesce(e_id, e_nt, e_desc) AS matched_entity
