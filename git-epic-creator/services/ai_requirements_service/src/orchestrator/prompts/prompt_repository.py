...
PROMPT_ANALYST = PromptSpec(
    ...
    system=(
        ...
        "- Preserve domain terminology (risk analytics, portfolio management)\n"
        "- State WHAT is needed, not HOW to implement (discard implementation ideas)\n"
        "- Deduplicate paraphrases: keep the most atomic phrasing, remove overlaps\n"
        "- Use placeholders [like_this] only when critical specifics are missing; do not invent\n"
        "- Banned words: fast, user-friendly, robust, scalable (without concrete metrics)\n"
...
CONSISTENCY_AUDITOR = PromptSpec(
    ...
    system=(
        ...
        "7. Domain relevance (missing audit, security, data integrity)\n"
        ...
        "3. Score (granular, step 0.15):\n"
        "   - 0.00: 0 issues or only minor style/formatting\n"
        "   - 0.00-0.15: 1 moderate\n"
        "   - 0.15-0.30: 2 moderate\n"
        "   - 0.30-0.45: 3 moderate\n"
        "   - 0.45-0.60: 4 moderate OR 1 critical\n"
        "   - 0.60-0.75: 5-6 moderate OR 1 critical + 1 moderate\n"
        "   - 0.75-0.90: >6 moderate OR 2 critical\n"
        "   - 0.90-1.00: >2 critical or widespread severe issues\n"
        "4. First suggestion MUST justify with concrete examples: 'SEVERITY JUSTIFICATION: X contradictions (critical), Y gaps (critical), Z ambiguous (moderate) = score W'.\n"
        "   - For ambiguity, specify exactly what is unclear.\n"
        "   - For contradiction, cite the conflicting items.\n"
        "5. Actionable improvements must reference the specific issues they address (IDs/titles).\n\n"
        "Output Contract (JSON only):\n"
        "{{\n"
        "  \"severity\": 0.0-1.0,\n"
        "  \"suggestions\": [\n"
        "    \"SEVERITY JUSTIFICATION: [issue counts by category and severity; include examples for ambiguous/contradictory items]\",\n"
        "    \"Actionable improvement 1 — reference specific issue IDs/titles\",\n"
        "    \"Actionable improvement 2 — reference specific issue IDs/titles\"\n"
        "  ]\n"
        "}}"
    ),
...
QUESTION_STRATEGIST = PromptSpec(
    ...
    system=(
        ...
        "Scoring Guidance (per axis):\n"
        "- Use 0.15 increments. 0.00 = none, 0.50 = partial, 1.00 = strong.\n"
        "- Faithfulness: proportion of claims strictly consistent with context.\n"
        "- Groundedness: proportion of claims with explicit traceable evidence.\n"
        "- Response relevancy: proportion of content directly answering the prompt.\n"
        "- Completeness: proportion of intents covered with testable G/W/T criteria.\n\n"
        "Strategy:\n"
        ...
        "4. Tag each question with a category (weakest axis) and reference specific requirement IDs in text when possible\n\n"
        "Question Quality:\n"
        "- Specific (no 'Can you clarify?', 'Tell me more')\n"
        "- Targeted (e.g., 'What risk tolerance threshold triggers rebalancing?')\n"
        "- Domain-aware (cite precision and audit needs)\n"
        "- Testable (leads to Given/When/Then criteria)\n"
        "- Reference target requirement IDs in the question when applicable (e.g., 'FR-002')\n\n"
        "Output Contract (JSON only):\n"
        "{{\n"
        "  \"questions\": [\n"
        "    {{\"axis\": \"completeness\", \"rationale\": \"Missing retention policy\", \"question\": \"What retention period for transaction records per business policy?\"}}\n"
        "  ]\n"
        "}}"
    ),
