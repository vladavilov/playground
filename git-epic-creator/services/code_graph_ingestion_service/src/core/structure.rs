...
pub fn build_contains_edges(nodes: &[CodeNodeRecord]) -> Vec<EdgeRecord> {
    let mut by_file: std::collections::BTreeMap<String, Vec<CodeNodeRecord>> =
    ...
    for (_file_path, mut file_nodes) in by_file {
        file_nodes.sort_by(|a, b| {
            (a.start_line, -a.end_line, a.node_id.clone()).cmp(&(
                b.start_line,
                -b.end_line,
                b.node_id.clone(),
            ))
        ...
        let mut child_order_by_parent: std::collections::HashMap<String, i64> =
        ...
        for cur in file_nodes {
            ...
            if let Some(parent) = stack.last() {
                    ...
                    && (parent.start_line, parent.end_line) != (cur.start_line, cur.end_line)
                {
                    ...
                    md.insert(
                        "strategy".to_string(),
                        Value::String("span-nesting".to_string()),
                    );
