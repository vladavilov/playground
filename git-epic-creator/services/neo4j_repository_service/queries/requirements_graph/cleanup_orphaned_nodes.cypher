...
// Step 4: Remove any remaining nodes missing IN_PROJECT for THIS project only.
// Avoid whole-db scans by filtering on the project_id property produced by ingestion.
WITH p, total_orphaned_chunks, total_orphaned_entities, deleted AS total_orphaned_communities
CALL (p) {
  MATCH (n)
  WHERE (n:__Document__ OR n:__Chunk__ OR n:__Entity__ OR n:__Community__)
    AND n.project_id = p.id
    AND NOT exists((n)-[:IN_PROJECT]->(p))
  RETURN n
}
