...
os.environ["AZURE_OPENAI_EMBEDDING_DEPLOYMENT_NAME"] = "test-embed-deployment"
os.environ["AZURE_OPENAI_EMBEDDING_MODEL_NAME"] = "test-embed-model"
...
def test_pdf_extraction_workflow_agent_integration(
    ...
    mock_embedder_instance.get_embedding.side_effect = [[0.1, 0.2], [0.3, 0.4], [0.1, 0.2]]
...
def test_workflow_aggregates_results(
    ...
    settings = get_settings()
    settings.property_groups = [
        {"group_name": "group1", "rag_query": "Query 1", "properties": [{"name": "prop1"}]},
        {"group_name": "group2", "rag_query": "Query 2", "properties": [{"name": "prop2"}]},
    ]
    settings.TOP_K = 1

    mock_embedder_instance = MagicMock()
    mock_embedder_instance.get_embedding.side_effect = [[0.1, 0.2], [0.3, 0.4], [0.1, 0.2], [0.3, 0.4]]
    mock_azure_embedder.return_value = mock_embedder_instance

    type(mock_chat_model.response.return_value).content = PropertyMock(
        side_effect=[json.dumps({"prop1": "value1"}), json.dumps({"prop2": "value2"})]
    )
